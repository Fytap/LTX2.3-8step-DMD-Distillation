...
class TrainingStrategyConfigBase(BaseModel):
    ...
    name: Literal["text_to_video", "video_to_video", "distill_text_to_video"] = Field(
...
class ModelInputs:
    ...
    # Optional teacher inputs/outputs for distillation strategies.
    teacher_video: Modality | None = None
    teacher_audio: Modality | None = None
    teacher_video_pred: Tensor | None = None
    teacher_audio_pred: Tensor | None = None

    # Optional critic/fake-score outputs for DMD-style distribution matching.
    critic_video_pred: Tensor | None = None
    critic_audio_pred: Tensor | None = None
    fake_video_targets: Tensor | None = None
    fake_audio_targets: Tensor | None = None
...
class TrainingStrategy(ABC):
    ...
    def requires_teacher(self) -> bool:
        """Whether this strategy needs an additional frozen teacher transformer."""
        return False

    @property
    def teacher_model_path(self) -> str | None:
        """Optional path to a frozen teacher checkpoint used by distillation strategies."""
        return None

    @property
    def requires_critic(self) -> bool:
        """Whether this strategy trains an additional fake-score critic."""
        return False

    @property
    def critic_model_path(self) -> str | None:
        """Optional checkpoint path used to initialize the trainable fake-score critic."""
        return None

...
def _pad_audio_latents_collate(batch: list[dict[str, Any]]) -> dict[str, Any]:
    """Collate precomputed AV samples, padding variable-length audio latents in-batch."""
    try:
        audio_items = [sample.get("audio_latents", {}).get("latents") for sample in batch]
        if any(item is None for item in audio_items):
            return default_collate(batch)
        if not all(isinstance(item, torch.Tensor) and item.ndim == 3 for item in audio_items):
            return default_collate(batch)
        shapes = [tuple(item.shape) for item in audio_items]
        if len(set(shapes)) <= 1:
            return default_collate(batch)
        ...
        for sample in batch:
            sample = dict(sample)
            audio = dict(sample["audio_latents"])
            latents = audio["latents"]
            if latents.shape[1] < max_len:
                pad = latents.new_zeros(latents.shape[0], max_len - latents.shape[1], latents.shape[2])
                latents = torch.cat([latents, pad], dim=1)
            audio["latents"] = latents
            sample["audio_latents"] = audio
...
class TrainingStepOutput:
    ...
    metrics: dict[str, Tensor | float] | None = None
...
class LtxvTrainer:
    ...
    def train(  # noqa: PLR0912, PLR0915
        ...
        self._save_config()
        loss_history_path = Path(cfg.output_dir) / "loss_history.csv"
        if IS_MAIN_PROCESS and (not loss_history_path.exists() or not resuming):
            with open(loss_history_path, "w") as f:
                if self._training_strategy.requires_critic:
                    f.write(
                        "step,train_generator,generator_loss,critic_loss,learning_rate,critic_learning_rate,"
                        "step_time,sigma_mean,video_dmd_loss,audio_dmd_loss,dmd_gradient_norm,"
                        "audio_dmd_gradient_norm,video_distill_loss,audio_distill_loss,"
                        "video_transition_loss,audio_transition_loss,video_quality_loss,audio_quality_loss,"
                        "clean_video_loss,clean_audio_loss,dmd_weight_scale,video_critic_loss,audio_critic_loss,"
                        "train_critic\n"
                    )
                else:
                    f.write(
                        "step,loss,learning_rate,step_time,sigma_mean,video_distill_loss,audio_distill_loss,"
                        "video_transition_loss,audio_transition_loss,video_quality_loss,audio_quality_loss,"
                        "clean_video_loss,clean_audio_loss\n"
                    )
        ...
        with progress:
            ...
            for step in range(remaining_steps * cfg.optimization.gradient_accumulation_steps):
                ...
                if self._training_strategy.requires_critic:
                    if cfg.optimization.gradient_accumulation_steps != 1:
                        raise ValueError("DMD critic training currently requires gradient_accumulation_steps=1")

                    self._global_step += 1
                    if hasattr(self._training_strategy, "set_current_step"):
                        self._training_strategy.set_current_step(self._global_step)
                    if hasattr(self._training_strategy, "should_train_generator"):
                        train_generator = self._training_strategy.should_train_generator(self._global_step)
                    else:
                        ratio = getattr(self._training_strategy.config, "dfake_gen_update_ratio", 1)
                        train_generator = (self._global_step - 1) % ratio == 0
                    train_critic = (
                        self._training_strategy.should_train_critic(self._global_step)
                        if hasattr(self._training_strategy, "should_train_critic")
                        else True
                    )
                    ...
                    if train_generator:
                        ...
                        generator_output = self._training_step(batch, train_generator=True)
                        self._accelerator.backward(generator_output.loss.mean())
                        max_gen_norm = getattr(self._training_strategy.config, "max_grad_norm_generator", 0.0)
                        if self._accelerator.sync_gradients and max_gen_norm > 0:
                            self._accelerator.clip_grad_norm_(self._trainable_params, max_gen_norm)
                        self._optimizer.step()

                    if train_critic:
                        try:
                            critic_batch = next(data_iter)
                        except StopIteration:
                            data_iter = iter(self._dataloader)
                            critic_batch = next(data_iter)

                        if self._critic_optimizer is None:
                            raise ValueError("DMD critic optimizer was not initialized")
                        self._critic_optimizer.zero_grad(set_to_none=True)
                        critic_output = self._training_step(critic_batch, train_generator=False)
                        self._accelerator.backward(critic_output.loss.mean())
                        max_critic_norm = getattr(self._training_strategy.config, "max_grad_norm_critic", 0.0)
                        if self._accelerator.sync_gradients and max_critic_norm > 0:
                            self._accelerator.clip_grad_norm_(self._critic_trainable_params, max_critic_norm)
                        self._critic_optimizer.step()

                    # In DMD mode the user-facing "step" is the global generator/critic
                    # iteration, so schedule both learning rates by global step rather
                    # than by the less frequent generator optimizer updates.
                    if self._lr_scheduler is not None:
                        self._lr_scheduler.step()
                    if self._critic_lr_scheduler is not None and train_critic:
                    ...
                    critic_lr = self._critic_optimizer.param_groups[0]["lr"] if self._critic_optimizer is not None else 0.0
                    step_time = time.time() - step_start_time
                    generator_loss = (
                        generator_output.loss.detach().mean().item() if generator_output is not None else None
                    )
                    critic_loss = critic_output.loss.detach().mean().item() if critic_output is not None else None
                    step_loss = (generator_loss if generator_loss is not None else 0.0) + (
                        critic_loss if critic_loss is not None else 0.0
                    )
                    sigma_for_log = generator_output.sigma if generator_output is not None else critic_output.sigma
                    ...
                    if IS_MAIN_PROCESS:
                        loss_for_tracker = (
                            ...
                            else generator_output.loss
                        )
                        self._sigma_tracker.update(sigma_for_log.cpu().tolist(), loss_for_tracker.detach().cpu().tolist())
                        metrics = {
                            "train/loss": step_loss,
                            "train/generator_loss": generator_loss if generator_loss is not None else 0.0,
                            "train/critic_loss": critic_loss if critic_loss is not None else 0.0,
                            "train/learning_rate": current_lr,
                            "train/critic_learning_rate": critic_lr,
                            "train/step_time": step_time,
                            "train/global_step": self._global_step,
                            "train/train_generator": int(train_generator),
                            "train/train_critic": int(train_critic),
                        }
                        if generator_output is not None and generator_output.metrics:
                            metrics.update(
                                {
                                    f"train/{key}": self._metric_to_float(value)
                                    for key, value in generator_output.metrics.items()
                                }
                            )
                        if critic_output is not None and critic_output.metrics:
                            metrics.update(
                                {
                                    f"train/{key}": self._metric_to_float(value)
                                    for key, value in critic_output.metrics.items()
                                }
                            )
                        ...
                        with open(loss_history_path, "a") as f:
                            f.write(
                                f"{self._global_step},{int(train_generator)},"
                                f"{'' if generator_loss is None else f'{generator_loss:.8f}'},"
                                f"{'' if critic_loss is None else f'{critic_loss:.8f}'},{current_lr:.8e},{critic_lr:.8e},"
                                f"{step_time:.6f},{sigma_for_log.detach().float().mean().item():.8f},"
                                f"{metrics.get('train/video_dmd_loss', '')},"
                                f"{metrics.get('train/audio_dmd_loss', '')},"
                                f"{metrics.get('train/dmd_gradient_norm', '')},"
                                f"{metrics.get('train/audio_dmd_gradient_norm', '')},"
                                f"{metrics.get('train/video_distill_loss', '')},"
                                f"{metrics.get('train/audio_distill_loss', '')},"
                                f"{metrics.get('train/video_transition_loss', '')},"
                                f"{metrics.get('train/audio_transition_loss', '')},"
                                f"{metrics.get('train/video_quality_loss', '')},"
                                f"{metrics.get('train/audio_quality_loss', '')},"
                                f"{metrics.get('train/clean_video_loss', '')},"
                                f"{metrics.get('train/clean_audio_loss', '')},"
                                f"{metrics.get('train/dmd_weight_scale', '')},"
                                f"{metrics.get('train/video_critic_loss', '')},"
                                f"{metrics.get('train/audio_critic_loss', '')},"
                                f"{metrics.get('train/train_critic', '')}\n"
                            )

                    if disable_progress_bars and IS_MAIN_PROCESS and self._global_step % 20 == 0:
                        elapsed = time.time() - train_start_time
                        steps_done = self._global_step - initial_step
                        total_estimated = elapsed / steps_done * remaining_steps if steps_done > 0 else 0
                        logger.info(
                            f"Step {self._global_step}/{cfg.optimization.steps} - "
                            f"Gen: {generator_loss if generator_loss is not None else 'skip'}, "
                            f"Critic: {critic_loss if critic_loss is not None else 'skip'}, "
                            f"LR: {current_lr:.2e}/{critic_lr:.2e}, "
                            f"Time/Step: {step_time:.2f}s, "
                            f"Total Time: {total_estimated // 3600:.0f}h {(total_estimated % 3600) // 60:.0f}m",
                        )

                    del generator_output, critic_output
                    if "critic_batch" in locals():
                        del critic_batch
                ...
                with self._accelerator.accumulate(self._transformer):
                    ...
                    if IS_MAIN_PROCESS and is_optimization_step:
                        ...
                        with open(loss_history_path, "a") as f:
                            f.write(
                                f"{self._global_step},{step_loss:.8f},{current_lr:.8e},"
                                f"{step_time:.6f},{output.sigma.detach().float().mean().item():.8f},"
                                f"{metrics.get('train/video_distill_loss', '')},"
                                f"{metrics.get('train/audio_distill_loss', '')},"
                                f"{metrics.get('train/video_transition_loss', '')},"
                                f"{metrics.get('train/audio_transition_loss', '')},"
    ...
    @staticmethod
    def _metric_to_float(value: Tensor | float | int) -> float:
    ...
    def _training_step(
        ...
        if getattr(self._training_strategy, "uses_trajectory_distillation", False):
            model_inputs = self._rollout_trajectory_prefix(model_inputs)

        if self._training_strategy.requires_teacher and train_generator:
            if self._teacher_transformer is None:
                raise ValueError("Teacher transformer was not loaded")
            self._set_teacher_device(self._accelerator.device)
            teacher_video = model_inputs.teacher_video if model_inputs.teacher_video is not None else model_inputs.video
            teacher_audio = model_inputs.teacher_audio if model_inputs.teacher_audio is not None else model_inputs.audio
            with torch.no_grad():
                with self._accelerator.autocast():
                    teacher_video_pred, teacher_audio_pred = self._teacher_transformer(
                        video=teacher_video,
                        audio=teacher_audio,
                        perturbations=None,
                    )
            model_inputs.teacher_video_pred = teacher_video_pred
            model_inputs.teacher_audio_pred = teacher_audio_pred

        if self._training_strategy.requires_critic:
            if self._critic_transformer is None:
                raise ValueError("Critic transformer was not loaded")

            if train_generator:
                self._transformer.train()
                self._critic_transformer.eval()

                with torch.no_grad():
                    with self._accelerator.autocast():
                        critic_video_pred, critic_audio_pred = self._critic_transformer(
                            video=model_inputs.video,
                            audio=model_inputs.audio,
                            perturbations=None,
                        )
                model_inputs.critic_video_pred = critic_video_pred
                model_inputs.critic_audio_pred = critic_audio_pred
                ...
                loss, metrics = self._training_strategy.compute_generator_loss(video_pred, audio_pred, model_inputs)
            else:
                self._transformer.eval()
                self._critic_transformer.train()
                # Critic-only DMD steps do not use the teacher. Keeping the frozen
                # teacher resident beside the student and critic is what pushes
                # 720p AV critic backward over H200 memory, so offload it before
                # the critic graph is built and reload on the next generator step.
                self._set_teacher_device("cpu")

                with torch.no_grad():
                    with self._accelerator.autocast():
                        fake_video_pred, fake_audio_pred = self._transformer(
                            video=model_inputs.video,
                            audio=model_inputs.audio,
                            perturbations=None,
                        )
                    model_inputs.fake_video_targets = self._training_strategy._to_x0(
                        model_inputs.video,
                        fake_video_pred,
                    ).detach()
                    if fake_audio_pred is not None and model_inputs.audio is not None:
                        model_inputs.fake_audio_targets = self._training_strategy._to_x0(
                            model_inputs.audio,
                            fake_audio_pred,
                        ).detach()

                    # The frozen generator pass only supplies fake x0 targets for
                    # the critic. Free its large AV predictions before entering
                    # the trainable critic forward; at 720p this avoids rank-local
                    # VRAM spikes without changing the objective.
                    del fake_video_pred, fake_audio_pred
                    gc.collect()
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                ...
                loss, metrics = self._training_strategy.compute_critic_loss(
                    critic_video_pred,
                    critic_audio_pred,
                    model_inputs,
                )
            ...
            return TrainingStepOutput(loss=loss, sigma=sigma, metrics=metrics)
        ...
        # Use strategy to compute loss. Distillation strategies can return richer
        # metrics even when they do not train a critic.
        metrics = None
        if self._training_strategy.requires_teacher and hasattr(self._training_strategy, "compute_generator_loss"):
            loss, metrics = self._training_strategy.compute_generator_loss(video_pred, audio_pred, model_inputs)
        else:
    ...
    def _rollout_trajectory_prefix(self, model_inputs):
        """Advance the student with no grad to the selected trajectory segment."""
        prefix_sigmas = self._training_strategy.trajectory_prefix_sigmas(model_inputs)
        if len(prefix_sigmas) <= 1:
            return model_inputs
        ...
        try:
            with torch.no_grad():
                with self._accelerator.autocast():
                    for current_sigma, next_sigma in zip(prefix_sigmas[:-1], prefix_sigmas[1:]):
                        model_inputs.video = self._training_strategy.with_sigma(
                            model_inputs.video,
                            current_sigma,
                        )
                        if model_inputs.audio is not None:
                            model_inputs.audio = self._training_strategy.with_sigma(
                                model_inputs.audio,
                                current_sigma,
                            )
                        video_pred, audio_pred = self._transformer(
                        ...
                        next_video = self._training_strategy.euler_step(model_inputs.video, video_pred, next_sigma)
                        model_inputs.video = self._training_strategy.with_sigma(
                            model_inputs.video,
                            next_sigma,
                            next_video.detach(),
                        )
                        if model_inputs.audio is not None and audio_pred is not None:
                            next_audio = self._training_strategy.euler_step(model_inputs.audio, audio_pred, next_sigma)
                            model_inputs.audio = self._training_strategy.with_sigma(
                                model_inputs.audio,
                                next_sigma,
                                next_audio.detach(),
        ...
        model_inputs.teacher_video = model_inputs.video
        model_inputs.teacher_audio = model_inputs.audio
        return model_inputs
    ...
    def _load_models(self) -> None:
        ...
        self._teacher_transformer = None
        self._critic_transformer = None
        if self._training_strategy.requires_teacher:
            teacher_path = self._training_strategy.teacher_model_path
            if teacher_path is None:
                raise ValueError("Training strategy requires a teacher but did not provide teacher_model_path")
            logger.info(f"Loading frozen teacher transformer from {teacher_path}")
        ...
        if self._training_strategy.requires_critic:
            critic_path = self._training_strategy.critic_model_path
            if critic_path is None:
                raise ValueError("Training strategy requires a critic but did not provide critic_model_path")
            logger.info(f"Loading trainable fake-score critic transformer from {critic_path}")
            self._critic_transformer = load_transformer(
        ...
        if self._teacher_transformer is not None:
            self._teacher_transformer.requires_grad_(False)
            self._teacher_transformer.eval()
        if self._critic_transformer is not None:
            self._critic_transformer.requires_grad_(False)
    ...
    def _collect_trainable_params(self) -> None:
        ...
        if self._config.model.training_mode == "lora":
            ...
            self._transformer = self._setup_lora(self._transformer, label="generator")
            if self._training_strategy.requires_critic:
                if self._critic_transformer is None:
                    raise ValueError("DMD strategy requires a critic transformer")
                self._critic_transformer = self._setup_lora(self._critic_transformer, label="critic")
        ...
        self._critic_trainable_params = (
            [p for p in self._critic_transformer.parameters() if p.requires_grad]
            if self._critic_transformer is not None
            else []
        )
        logger.debug(f"Trainable params count: {sum(p.numel() for p in self._trainable_params):,}")
        if self._critic_trainable_params:
            logger.debug(f"Critic trainable params count: {sum(p.numel() for p in self._critic_trainable_params):,}")
    ...
    def _setup_lora(self, transformer: torch.nn.Module, label: str = "generator") -> torch.nn.Module:
        """Configure LoRA adapters for the transformer. Only called in LoRA training mode."""
        logger.debug(f"Adding {label} LoRA adapter with rank {self._config.lora.rank}")
    ...
    def _load_lora_checkpoint(self, checkpoint_path: Path) -> None:
        ...
        # The installed PEFT version expects a newer Transformers tensor-parallel
        # symbol even when this model is not using tensor parallelism. Disable the
        # optional TP sharding hook for this non-TP trainer path.
        try:
        ...
        self._load_lora_into_model(self._transformer, checkpoint_path, label="generator")

        if self._critic_transformer is not None:
            match = re.search(r"lora_weights_step_(\d+)", checkpoint_path.name)
            critic_path = None
            if match:
                critic_path = checkpoint_path.parent / f"critic_lora_weights_step_{match.group(1)}.safetensors"
            if critic_path is not None and critic_path.exists():
                self._load_lora_into_model(self._critic_transformer, critic_path, label="critic")
            else:
                logger.warning(
                    f"Critic LoRA checkpoint not found for {checkpoint_path}; "
                    "critic will resume from fresh initialization."
                )

        logger.info("✅ LoRA checkpoint loaded successfully")

    @staticmethod
    def _load_lora_into_model(model: torch.nn.Module, checkpoint_path: Path, label: str) -> None:
        state_dict = load_file(checkpoint_path)
        ...
        logger.info(f"Loaded {label} LoRA checkpoint from {checkpoint_path}")
    ...
    def _resolve_resume_state(self) -> tuple[int, TrainingState | None]:
        ...
        if fp.scheduler_type != cfg.optimization.scheduler_type:
            allow_dmd_lr_fix = (
                self._training_strategy.requires_critic
                and fp.scheduler_type == "step"
                and cfg.optimization.scheduler_type == "constant"
            )
            if allow_dmd_lr_fix:
                logger.warning(
                    "Accepting scheduler_type mismatch for DMD LR correction: "
                    f"{fp.scheduler_type} -> {cfg.optimization.scheduler_type}"
                )
            else:
    ...
    def _set_teacher_device(self, device: torch.device | str) -> None:
        """Move the frozen teacher only when DMD needs to reclaim VRAM."""
        if self._teacher_transformer is None:
            return
        current_device = next(self._teacher_transformer.parameters()).device
        target_device = torch.device(device)
        if current_device == target_device:
            return
        self._teacher_transformer = self._teacher_transformer.to(device=target_device, dtype=torch.bfloat16)
        self._teacher_transformer.eval()
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    def _prepare_models_for_training(self) -> None:
        ...
        if self._accelerator.distributed_type == DistributedType.FSDP and self._config.model.training_mode == "lora":
            ...
            if self._critic_transformer is not None:
                self._critic_transformer = self._critic_transformer.to(dtype=torch.float32)
        ...
        if self._critic_transformer is not None:
            critic_transformer = (
                self._critic_transformer.get_base_model()
                if hasattr(self._critic_transformer, "get_base_model")
                else self._critic_transformer
            )
            critic_transformer.set_gradient_checkpointing(self._config.optimization.enable_gradient_checkpointing)

        if self._teacher_transformer is not None:
            self._teacher_transformer = self._teacher_transformer.to(
                device=self._accelerator.device,
                dtype=torch.bfloat16,
            )
            self._teacher_transformer.eval()
        ...
        if self._critic_transformer is not None:
            self._critic_transformer = self._accelerator.prepare(self._critic_transformer)
    ...
    def _init_dataloader(self) -> None:
        ...
        dataloader = DataLoader(
            ...
            collate_fn=_pad_audio_latents_collate,
        )
    ...
    def _init_optimizer(self) -> None:
        ...
        self._critic_lr_scheduler = None
        if self._training_strategy.requires_critic:
            if not self._critic_trainable_params:
                raise ValueError("DMD strategy requires critic trainable parameters")
            critic_lr = getattr(self._training_strategy.config, "critic_learning_rate", None) or lr
            if opt_cfg.optimizer_type == "adamw":
                critic_optimizer = AdamW(self._critic_trainable_params, lr=critic_lr)
            elif opt_cfg.optimizer_type == "adamw8bit":
                from bitsandbytes.optim import AdamW8bit  # noqa: PLC0415

                critic_optimizer = AdamW8bit(self._critic_trainable_params, lr=critic_lr)
            else:
                raise ValueError(f"Unknown optimizer type: {opt_cfg.optimizer_type}")
            critic_lr_scheduler = self._create_scheduler(critic_optimizer)
            self._critic_optimizer, self._critic_lr_scheduler = self._accelerator.prepare(
                critic_optimizer,
                critic_lr_scheduler,
            )
    ...
    def _save_checkpoint(self) -> Path | None:
        ...
        def unwrap_without_accelerate(model):
            while hasattr(model, "module"):
                model = model.module
            return model

        # Get state dict (collective operation - all processes must participate).
        # For LoRA under regular DDP, avoid Accelerator.unwrap_model/get_state_dict:
        # Accelerate may import system deepspeed during unwrapping, and this host has
        # a broken deepspeed->xgboost install that crashes checkpoint saving.
        self._accelerator.wait_for_everyone()
        full_state_dict = None if is_lora and not is_fsdp else self._accelerator.get_state_dict(self._transformer)
        critic_full_state_dict = None
        if self._critic_transformer is not None and (not is_lora or is_fsdp):
            critic_full_state_dict = self._accelerator.get_state_dict(self._critic_transformer)
        ...
        if is_lora:
            unwrapped = (
                self._accelerator.unwrap_model(self._transformer, keep_torch_compile=False)
                if is_fsdp
                else unwrap_without_accelerate(self._transformer)
            )
            ...
            if self._critic_transformer is not None:
                critic_unwrapped = (
                    self._accelerator.unwrap_model(
                        self._critic_transformer,
                        keep_torch_compile=False,
                    )
                    if is_fsdp
                    else unwrap_without_accelerate(self._critic_transformer)
                )
                critic_state_dict = get_peft_model_state_dict(
                    critic_unwrapped,
                    state_dict=critic_full_state_dict if is_fsdp else None,
                )
                critic_state_dict = {k.replace("base_model.model.", "", 1): v for k, v in critic_state_dict.items()}
                critic_state_dict = {f"diffusion_model.{k}": v for k, v in critic_state_dict.items()}
                critic_state_dict = {
                    k: v.to(save_dtype) if isinstance(v, Tensor) else v for k, v in critic_state_dict.items()
                }
                critic_path = save_dir / f"critic_lora_weights_step_{self._global_step:05d}.safetensors"
                save_file(critic_state_dict, critic_path, metadata={**metadata, "checkpoint_role": "dmd_fake_score_critic"})
        else:
            # Cast to configured precision
